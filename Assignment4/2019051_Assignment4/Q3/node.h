...
class Node {
 ...
 public:
  ...
  void updateTblEntry(string destination_ip){

    for(int i = 0; i<mytbl.tbl.size(); i++){
      if(mytbl.tbl[i].dstip==destination_ip){
        mytbl.tbl[i].cost=16;
      }
    }

    for(int i = 0; i<interfaces.size(); i++){
      if(interfaces[i].first.getConnectedIp()==destination_ip){
